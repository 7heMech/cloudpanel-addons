...
/**
 * What lets CloudPanel's header row wrap.
 *
 * The panel lays it out as one non-wrapping flex row of a fixed 75px, sized for
 * the three things in it: a 235px logo, the navigation, and the tools. Nothing
 * else fits, and nothing moves onto a second line. Flattening the right-hand
 * wrapper makes its children items of the header itself, which is what lets the
 * row wrap at all.
 *
 * Both the update notice and Panel Tweaks' narrow-screen header need this, and
 * each wants it under different conditions, so the header's own selector is the
 * argument: the manager asks for it only while a notice is in the row, and the
 * addon asks for it whenever its switch is on.
 */
export function headerWrapStyle(header: string): string {
  return `
${header} { height: auto; min-height: 75px; flex-wrap: wrap; align-items: stretch; }
${header} .navbar-right-container { display: contents !important; }
${header} .nav-link-container,
${header} .header-instance-information-container { flex: 1 1 0; min-width: 0; white-space: nowrap; }
${header} .header-instance-information-container { overflow-x: auto; }
${header} .header-instance-information-container > ul { float: none; width: max-content; min-width: 100%;
  padding: 26px 20px; }
${header} .navbar-right { flex: 0 0 auto; padding-left: 0; margin-left: auto; }
`;
}
...
// room for it, and on a line of its own once there is not.
const PANEL_UPDATE_STYLE = UPDATE_STYLE + PANEL_HEADER_STYLE + `
