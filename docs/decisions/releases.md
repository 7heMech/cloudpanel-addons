...
answer is held in memory for fifteen minutes, refreshed by one call at a time,
and served as it stands while that refresh runs. Only the first check after a
start waits for GitHub: without that, a page load on a box with no route to
GitHub paid the full timeout once per request it made, and spent one of the
sixty anonymous API calls an hour that address is allowed on each.
