...
| Panel UI tweaks | Small additions to CloudPanel's own pages: a site count, search, sorting, SSL, runtime and measured size columns, a Sites table that reads on a phone, row actions in a menu, the panel's own header and Dashboard fitted to a narrow screen, and a device theme on the login page. |
